...
class FAQHandlerTests(unittest.TestCase):
	...
	# Mocked test: FAQHandler get()
	@patch.object(webapp2.RequestHandler, 'redirect')
	def testFAQHandlerGet(self, mock):
		# Arrange: Make the web request. 
		test_request = webapp2.Request.blank('/login', POST={"firstname" : "TESTFIRST", "lastname" : "TESTLAST", "email" : "fake_useremail@.com", "password" : "PASSWORD", "b9" : "COMPSCI101"})
	...
	# Mocked test: FAQHandler post() Back to FAQ page
	@patch.object(webapp2.RequestHandler, 'redirect')
	def testBackToFAQPost(self, mock): 
		# Arrange: Make the web request. 
